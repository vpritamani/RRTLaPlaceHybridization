...
def LaPlace_average(potential_map, kernel, height, length, boundary_x, boundary_y, end, node_list, height_always, length_always, node_list_shape, indices):
...
def RRT(image, start, end, iterations, step_size, file_name, prob_of_choosing_start, la_place_at_start, la_place_each_time, show_every_attempted_point, show_expansion, branches_before_each_laplace, visualization, output_path):
    ...
    for _ in range(la_place_at_start):
        ...
        potential_map = LaPlace_average(potential_map, kernel, height, length, boundary_x, boundary_y, end, node_list, height_always, length_always,node_list_shape, indices)
    ...
    while pathFound == False:
        ...
        # new started here
        potential_map_as_image = np.uint8(255 * potential_map)
        gradient_map = np.zeros_like(potential_map_as_image, dtype=np.float32)
        for row in range(1, potential_map_as_image.shape[0] - 1):
            for col in range(1, potential_map_as_image.shape[1] - 1):
                gradr = potential_map[row + 1, col] - potential_map[row - 1, col]  # Gradient in the row direction
                gradc = potential_map[row, col + 1] - potential_map[row, col - 1]  # Gradient in the column direction
                maggrad = math.sqrt(gradr**2 + gradc**2)
                gradient_map[row, col] = maggrad
                ...
                #       toPut = 250 - int(gradient_map[y][x]*225)
                #       result.putpixel((x, y), (toPut, toPut, toPut))
                # result.show()
        # plt.imshow(gradient_map)
        # plt.show()
        # fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
        # surf = ax.plot_surface(np.indices(potential_map_as_image.shape)[0], np.indices(potential_map_as_image.shape)[1], gradient_map, cmap=cm.coolwarm,
        ...
        #       result.putpixel((x, y), (toPut, toPut, toPut))
        # result.save('pot_map.png')
        gradient_map[gradient_map < 0.001] = 0
        gradient_map[gradient_map != 0] = 255
        gradient_map = np.uint8(gradient_map)
        # fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
        # surf = ax.plot_surface(np.indices(potential_map_as_image.shape)[0], np.indices(potential_map_as_image.shape)[1], gradient_map, cmap=cm.coolwarm,
        #                linewidth=0, antialiased=False)
        # fig.colorbar(surf, shrink=0.5, aspect=5)
        # plt.show()
        # plt.savefig('gradient_after_seg_map.png')
        # im = Image.open(file_name)
        # result = im.copy()
        # for x in range(0, length):
        #   for y in range(0, height):
        #       toPut = 250 - int(potential_map[y][x]*225)
        #       result.putpixel((x, y), (toPut, toPut, toPut))
        # result.save('reset_boundary_conds_map.png')
        edge = cv2.Canny(gradient_map, 50, 150)
        edge_points = np.argwhere(edge > 0)
...
def try_grad_descent(potential_map, step_size, new_x, new_y, node_list, nodes_array):
    ...
    while True:
        ...
        if 0 < posec < potential_map.shape[1]-1 and 0 < poser < potential_map.shape[0]-1:
            nearest_nodes = list(nodes_array.intersection((posec - 1 , poser - 1, posec + 1, poser + 1), objects=True))
        valid = []
        if nearest_nodes:
...
images_to_test = ["world3"]#["world3", "world4"]#["spiral_image"]#
start =  "(1, 1)" #"(254, 180)"#
end =  "(650, 350)" #"(1, 1)"#
iterations = 1000
step_size = 1
laplace_iters_to_test = [50]#[50, 500, 100, 200, 300, 500]
branches_before_each_laplace = [1]#[1, 5, 10, 25, 50]
tests_of_each = 1
